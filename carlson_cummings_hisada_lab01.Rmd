...
We are alsso seeing relatively strong negative correlations between martial status-releated variables `PercentMarried` and `PctMarriedHouseholds` and `deathRate`. Marriage somehow decreases the chance of dying from cancer. Similary to racial groups, however, after looking at the correlations between the martial-status variables and the same set of variables strongly correlated with `deathRate`, we see married people are less much likely to be in poverty which explains the negative correlation observed rather than people's martial statuses themselves. 
